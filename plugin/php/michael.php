 ...
 * Created by PhpStorm.
 * User: james_000
 * Date: 5/8/2016 modified 5/12/16 by Michael Gorsuch
 * Time: 4:20 PM
 */
    include 'connect.php';
...
$eventType=NULL, $topics=NULL, $state=NULL, $city=NULL, $zip=NULL)
{
    ...
    unset($sql); // make sure the array is empty every time we call 
...
this
    ...
    if($endDate==NULL)
    {
    $endDate=date('Y-m-d', strtotime("+3 months", strtotime
...
($startDate))); 
    ...
    if($eventType)
    {
        $sql[] = " MAINTYPE = '$eventType' ";
    echo "MAINTYPE ".$eventType;
    }
    if($topics)
    {
        $sql[] = " MAINTOPIC = '$topics' ";
    ...
    //use implode to append filter parameters to query
     if(!empty($sql)) {
        $query .= ' WHERE Start_Time BETWEEN '."'".$startDate."'".' AND 

'."'".$endDate."'".' AND '.implode(' AND ', $sql);
    ...
    $result = mysqli_query($db, $query) or die("Error in Selecting " . 
...
//testing for dates
//echo getEvents('2016-02-01','2016-04-30', NULL, NULL, NULL, NULL, 
...
//test with maintype
//echo getEvents('2016-03-01','2016-03-30',"Get 

//Informed",NULL,NULL,NULL,NULL);

//testing for filtering for city "Kasson"
//echo getEvents(NULL, NULL, NULL, NULL, NULL, 'Kasson', NULL);

//testing for "Kasson" and dates
echo getEvents('2016-01-01','2016-04-30',NULL,NULL, NULL, 'Kasson', 
